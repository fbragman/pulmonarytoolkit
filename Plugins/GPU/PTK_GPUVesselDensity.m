classdef PTK_GPUVesselDensity < PTKPlugin
    ...
    methods (Static)
        ...
        function results = RunPlugin(dataset, ~)    
            lung_mask = dataset.GetResult('PTK_GPULeftAndRightLungs');
            lung_mask.ChangeRawImage(uint8(lung_mask.RawImage > 0));
            vesselness = dataset.GetResult('PTK_GPUVesselness');
