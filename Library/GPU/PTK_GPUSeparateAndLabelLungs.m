function both_lungs = PTK_GPUSeparateAndLabelLungs(unclosed_lungs, filtered_threshold_lung, lung_roi, reporting)
...
function success = SeparateLungs(both_lungs, lung_roi, unclosed_lungs, max_iter, is_coronal, reporting)
    ...
    while (length(largest_areas_indices) < 2) || (largest_area_numpixels(2) < minimum_required_voxels_per_lung)
        ...
        % Morphological opening on the GPU
        image_to_close.BinaryMorphWithGPU(@imopen, opening_size);
