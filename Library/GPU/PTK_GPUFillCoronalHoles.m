function lung_image = PTK_GPUFillCoronalHoles(lung_image, is_right, reporting)
    % PTK_GPUFillCoronalHoles. Operates on each coronal slice, applying a closing
    % filter then filling interior holes using the GPU 
    %
    %
    %
    %     Licence
    ...
    for coronal_index = 11 : lung_image.ImageSize(1) - 10
        ...
        coronal_slice = gather(imclose(gpuArray(coronal_slice), strel('disk', round(closing_size))));
...
function mask = OpenOrClose(mask, is_right, opening_size, closing_size, reporting)
    closed_image = gather(imclose(gpuArray(mask), strel('disk', round(closing_size))));
    opened_image = gather(imopen(gpuArray(mask), strel('disk', round(opening_size))));
