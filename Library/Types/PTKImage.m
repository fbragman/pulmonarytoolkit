classdef (ConstructOnLoad = true) PTKImage < handle
    ...
    methods
        ...
        function BinaryMorphWithGPU(obj, morph_function_handle, size_mm)
            ...
            % Convert RawImage to gpuArray
            RawImage_gpu = gpuArray(obj.RawImage);
            
            gpu_res = logical(morph_function_handle(RawImage_gpu, ball_element));
            
            % Gather gpuArray
            obj.RawImage = gather(gpu_res);
            obj.NotifyImageChanged;
        end
        ...
        function MorphWithBorderWithGPU(obj, morph_function_handle, size_mm)
            ...
            % Convert image to gpuArray
            morphed_image_gpu = gpuArray(morphed_image);
            
            gpu_res = uint8(morph_function_handle(morphed_image_gpu, ball_element));
            
            morphed_image = gather(gpu_res);
